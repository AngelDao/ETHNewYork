...
const rinkeby = "https://rinkeby.infura.io/v3/";
...
const getOfferOrConsiderationItem = (
  ...
  recipient
    ...
    const offerItem = {
      ...
      startAmount: ethers.BigNumber.from(startAmount),
      endAmount: ethers.BigNumber.from(endAmount),
    };
...
const getItemETH = (
	...
	getOfferOrConsiderationItem(
		...
		0,
		ethers.BigNumber.from(startAmount),
		ethers.BigNumber.from(endAmount),
		recipient
	);
...
const signOrder = async (
  ...
  const { domainSeparator } = await seaportContract.information();
...
const generateAndSignOrder = (voucherData) => {
  ...
  const offerer = await signer.getAdress();
 ...
 // for lazy minting of an NFT collection we will make an array of 100 NFTs and pass in 0 for the id criteria and address will be the same for 1155
  ...
  for (var i = 0; i < 100; i++){
    offers.push(
      getTestItem721(0, storeFrontAddress)
    );
  }
  ...
  const order = [
    ...
    "PARTIAL", //type
    [], //criteria resolvers
    blockNumber, //startTime,
    blockNumber + (30 * 5366), //endTime, 30 days worth of blocks
    zoneHash, // zone hash
    ethers.constants.HashZero, //blank salt
    ethers.constants.HashZero, //blank conduit key
    counter //counter
  ];
 ...
 const orderComponents = [
  ...
  const signature = await signOrder(orderComponents, orderHash, signer, seaportContract);
...
const partialFulfillOrder = async (advancedOrder) => {
	...
	const recipient = await signer.getAdress();

	const tx = await seaportContract.fulfillAdvancedOrder(advancedOrder, criteriaResolvers, fulfillerConduitKey, recipient);
